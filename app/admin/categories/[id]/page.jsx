...
export default function CategoryDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/categories")
      .then((r) => r.json())
      .then((data) => setCategories((data.data || data || []).filter((c) => c.id !== id)))
      .catch(() => {});
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/categories/${id}`, {
        ...
        body: JSON.stringify({ name: form.name, slug: form.slug, description: form.description, parentId: form.parentId, isActive: form.isActive }),
      });
      if (!res.ok) {
  ...
  if (!form) return <div style={{ padding: 40, textAlign: "center" }}>{error || "Loading..."}</div>;
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, maxWidth: 700 }}>
        ...
        <div>
          ...
          <select value={form.parentId || ""} onChange={(e) => update("parentId", e.target.value || null)} style={inputStyle}>
      ...
      <div style={{ marginTop: 24, display: "flex", gap: 12 }}>
        <button onClick={handleSave} disabled={saving} style={{ padding: "10px 24px", background: "#2563eb", color: "white", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 500, opacity: saving ? 0.6 : 1 }}>
          {saving ? "Saving..." : "Save Changes"}
        </button>
        <button onClick={handleDelete} disabled={deleting} style={{ padding: "10px 24px", background: "#dc2626", color: "white", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 500, opacity: deleting ? 0.6 : 1 }}>
